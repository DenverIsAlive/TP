...
if (file_exists($cacheFile)) {
  ...
  // Convert expire string back to timestamp
        $expireTimestamp = DateTime::createFromFormat('d/m/Y h:i A', $cacheData['expire']);
        if ($expireTimestamp && $expireTimestamp->getTimestamp() > time()) {
...
$JsonFile = fetchJson('secure/TP_Custom.json');
if ($JsonFile === false) {
...
foreach ($fetcherData as $channel) {
    if ((string)$channel['channel_id'] === (string)$id) {
        $channelData = $channel;
        break;
    }
}
...
$JwtToken = $wv_token['data']['token'] ?? null;

if (!$JwtToken) {
    echo json_encode(['error' => $wv_token], JSON_PRETTY_PRINT);
    exit;
}
